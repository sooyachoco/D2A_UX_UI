...
> AiDitor 작업에서 처음 만들어진 메타 인프라를 D2A 표준으로 도입.
...
[인스턴스층] specs/{NNN}-{feature}/                 ← 실제 프로젝트 산출물
      디폴트를 한 번 적용한 결과 (AiDitor·NEXON STREAM 등)
```
...
특정 프로젝트(예: AiDitor)에만 의미 있는 인사이트가 디폴트에 잘못 박히면 다른 archetype(백오피스·CLI·API)에서 마찰 유발.
