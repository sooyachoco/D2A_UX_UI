...
import { updateState } from "./tools/state-tool.js";
...
const TOOLS = [
    {
        ...
        description: "Phase 전환 가능 여부를 코드로 검증한다. " +
    ...
    },
    {
        ...
        description: "tasks.md의 done 기준을 독립적으로 재실행하여 태스크 완료를 검증한다. " +
    ...
    {
        name: "update_state",
        description: ".claude/state.json을 atomic write로 부분 업데이트한다. " +
    ...
    },
    {
        ...
        description: "태스크 실행 전 현재 HEAD에 checkpoint 브랜치를 생성한다. " +
    ...
    },
    {
        ...
        description: "태스크 실패(done 기준 2회 실패) 시 checkpoint 시점으로 recovery 브랜치를 생성하여 복원한다. " +
    ...
    {
        name: "get_next_task",
        description: "지정된 Phase에서 다음에 실행할 태스크를 결정하여 반환한다. " +
    ...
    {
        name: "submit_task",
        description: "태스크 구현 완료를 제출하여 done 기준을 검증하고 다음 액션을 결정한다. " +
    ...
    },
    {
        ...
        description: "prerequisites.md의 ⬜ 차단 항목이 정책 문서(refs/policies/)로 뒷받침되는지 검증한다. " +
...
// ─── MCP 서버 초기화 ───────────────────────────────────────────────────────
const server = new Server({ name: "d2a-harness", version: "1.0.0" }, { capabilities: { tools: {} } });
    ...
    try {
        ...
        switch (name) {
            ...
            case "update_state": {
                ...
                result = await updateState({ patch: args.patch });
                recordTrace(name, args, result);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
