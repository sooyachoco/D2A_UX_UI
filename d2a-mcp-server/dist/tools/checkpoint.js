 ...
 */
export async function rollbackToCheckpoint(args) {
    const { task_id } = args;
    const cwd = process.cwd();
    if (!isGitRepo(cwd)) {
        return { ok: false, error: "git 저장소가 아닙니다" };
    ...
    try {
        ...
        return { ok: false, error: `checkpoint 브랜치 SHA 조회 실패: ${e.message}` };
    }
    ...
    try {
        const status = execSync("git status --porcelain", { cwd, stdio: "pipe" })
            .toString().trim();
        if (status) {
            execSync(`git stash push -m "d2a-rollback-${task_id}-${Date.now()}"`, { cwd, stdio: "pipe" });
        }
    }
    catch (stashErr) {
        return {
            ok: false,
            error: `uncommitted 변경이 있으나 git stash 실패 — 수동으로 변경 사항을 처리한 뒤 재시도: ${stashErr.message ?? String(stashErr)}`,
        };
    }
