 ...
 *   그 외 → 셸 명령으로 실행
 */
async function executeCriterion(criterion, cwd) {
    ...
    if (containsTyped) {
        ...
        try {
            ...
            return { passed: false, reason: `파일 읽기 실패: ${filePart.trim()} — ${e.message}` };
        }
    }
    // regex: {path} :: {pattern}
    ...
    if (regexTyped) {
        ...
        try {
            ...
            try {
                ...
                return { passed: false, reason: `잘못된 정규식: "${regexStr.trim()}" — ${e.message}` };
            }
    ...
    if (jsonTyped) {
        ...
        try {
            ...
            return { passed: false, reason: `JSON 읽기/파싱 실패: ${filePart.trim()} — ${e.message}` };
        }
    }
    ...
    if (patternMatch) {
        ...
        try {
            ...
            return { passed: false, reason: `파일 읽기 실패: ${filePart} — ${e.message}` };
        }
    }
 ...
 */
function checkCoverageReport(sourcePath, threshold, cwd) {
    ...
    if (!istanbulReport && !pytestReport) {
        return {
            passed: false,
            reason: `coverage: 리포트 없음 — done 기준에 테스트 실행 cmd를 먼저 추가하세요.\n` +
...
function parseIstanbulReport(reportPath, sourcePath, threshold, cwd) {
    let report;
    try {
        report = JSON.parse(fs.readFileSync(reportPath, "utf-8"));
    }
    catch (e) {
        return { passed: false, reason: `coverage: 리포트 파싱 실패 — ${e.message}` };
    }
...
function parsePytestCovReport(reportPath, sourcePath, threshold, cwd) {
    let report;
    try {
        report = JSON.parse(fs.readFileSync(reportPath, "utf-8"));
    }
...
function formatCoverageResult(pct, threshold, sourcePath, scope) {
 ...
 */
function checkUtReport(reportPath, criteriaStr, cwd) {
    ...
    for (const level of ["S1", "S2", "S3", "S4"]) {
        ...
        if (m)
            counts[level] = parseInt(m[1], 10);
    }
    if (Object.keys(counts).length === 0) {
        return {
            passed: false,
            reason: `ut: ${reportPath} 에서 Severity 카운트(S1~S4)를 찾지 못함 — ` +
    ...
    for (const rule of rules) {
        ...
        if (!ok)
            failures.push(`${level}=${actual} 위반 (요구: ${op}${expected})`);
    }
 ...
 */
export async function validateTaskDone(args) {
    const { task_id } = args;
