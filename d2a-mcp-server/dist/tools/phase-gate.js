...
import { migrateState } from "../shared/state-schema.js";
...
function extractUnresolvedDecisions(decisionsPath) {
    try {
 ...
 */
function isCriticalDecision(line) {
 ...
 */
function detectProgressPlaceholders(progressPath) {
    try {
 ...
 */
export async function checkPhaseGate(args) {
    const { phase } = args;
    ...
    if (phase >= 1) {
        ...
        if (hasPhase05) {
            ...
            if (!found) {
                blockers.push("integration-ready.md 없음 — /collect-prerequisites 실행 후 재시도");
            }
            else {
                const content = fs.readFileSync(found, "utf-8");
                if (!content.includes("✅ AUTONOMOUS ZONE 진입 가능")) {
                    blockers.push(`integration-ready.md 판정이 "✅ AUTONOMOUS ZONE 진입 가능"이 아님 (${path.relative(cwd, found)})`);
                }
                else {
    ...
    if (phase >= 1 && stateData?.gnb_required === true) {
        ...
        if (!protoFile) {
            blockers.push("GNB 필수 프로젝트: GNB 스크립트 삽입 위치 없음 — prototype/index.html, frontend/index.html, 또는 frontend/src/app/layout.tsx 확인 필요");
        }
        else {
            ...
            if (missing.length > 0) {
                blockers.push(`GNB 스크립트 미삽입 (${path.relative(cwd, protoFile)}): ${missing.join(", ")} — ` +
    ...
    if (phase >= 1 &&
        ...
        for (const f of REQUIRED_INSIGN_FILES) {
            if (!fs.existsSync(path.join(cwd, f))) {
                blockers.push(`INSIGN 자산 누락: ${f} — create-spec Step 2.7 (L303-319)에서 InfaceTest 패턴 이식 필요`);
            }
        }
        ...
        if (fs.existsSync(authCtx)) {
            ...
            if (/MOCK_USER|localStorage\.getItem\(['"][\w-]*mock[\w-]*auth/i.test(src)) {
                blockers.push("Mock AuthContext 잔존 (frontend/src/context/AuthContext.tsx): MOCK_USER 또는 localStorage mock-auth 패턴 — " +
        ...
        if (!fs.existsSync(ssPath)) {
            blockers.push("storageState 없음 (tests/e2e/.auth/user.json) — ./scripts/save-auth-state.sh 실행 필요");
        }
        else {
            try {
                const ss = JSON.parse(fs.readFileSync(ssPath, "utf-8"));
                const nowSec = Math.floor(Date.now() / 1000);
                const hasValidCookie = (ss.cookies ?? []).some((c) => typeof c.expires === "number" && c.expires > nowSec);
                if (!hasValidCookie) {
                    blockers.push("storageState 만료 (tests/e2e/.auth/user.json): 유효한 인증 쿠키 없음 — " +
        ...
        if (stateData?.step27_user_approved !== true) {
            ...
            if (!hasApprovalEvent) {
                blockers.push("Step 2.7 사용자 승인 흔적 없음 — state.step27_user_approved=true 또는 " +
    ...
    if (phase >= 0.5 &&
        ...
        for (const t of targets) {
            ...
            if (hits.length > 0) {
                blockers.push(`auth_profile=${stateData.auth_profile}에 금지된 OAuth 키워드가 ${path.relative(cwd, t)}에 등장: ${hits.join(", ")} — ` +
        ...
        if (tasksFile) {
            try {
                ...
                if (prevTasks.length > 0 && incomplete.length > 0) {
                    blockers.push(`Phase ${prevPhase} 미완료 태스크 ${incomplete.length}개: ${incomplete
                        .map((t) => t.id)
                ...
                if (phase === 1) {
                    ...
                    if (phase05Tasks.length > 0 && incomplete05.length > 0) {
                        blockers.push(`Phase 0.5 미완료 태스크 ${incomplete05.length}개: ${incomplete05
                            .map((t) => t.id)
                            .join(", ")} — tasks.md ☑ 갱신 후 재시도`);
                    }
                }
    ...
    if (dFound) {
        ...
        for (const item of allUnresolved) {
            if (isCriticalDecision(item)) {
                blockers.push(`decisions.md 인프라·인증·보안 미결정: ${item} — 정책 문서(refs/INDEX.md) 참조 후 결정 필요`);
            }
            else {
    ...
    if (phase >= 2) {
        ...
        if (fs.existsSync(progressPath)) {
            ...
            for (const p of placeholders) {
                blockers.push(`PROGRESS.md "코드 패턴 메모" placeholder 잔존: ${p} — ` +
