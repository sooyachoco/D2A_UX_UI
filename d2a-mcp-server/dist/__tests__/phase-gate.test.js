...
describe("checkPhaseGate", () => {
    ...
    function setupGnbProject(gnbFileRelPath) {
        ({ dir, cleanup } = createTempGitRepo());
        cwdSpy = mockCwd(dir);
        mkdirSync(join(dir, ".claude"), { recursive: true });
        const fullPath = join(dir, gnbFileRelPath);
        mkdirSync(join(fullPath, ".."), { recursive: true });
        writeFileSync(fullPath, `<Script src="https://ssl-test.nexon.com/s1/global/ngb_head.js" />\n` +
    ...
    it("specs/.template/decisions.md의 🔴 미결정 placeholder는 게이트에 영향 없음", async () => {
        ...
        writeFileSync(join(dir, "specs/.template/decisions.md"), "| DEC-01 | {주제} | {결정 내용} | {왜 이 선택인지} | ✅ 확정 / 🔴 미결정 |\n", "utf-8");
        writeFileSync(join(dir, "specs/001-feature/decisions.md"), "| DB | ✅ PostgreSQL | 선택 완료 |\n", "utf-8");
        writeFileSync(join(dir, ".claude/state.json"), JSON.stringify({ ...DEFAULT_STATE, schema_version: STATE_SCHEMA_VERSION }), "utf-8");
        const result = await checkPhaseGate({ phase: 0 });
        expect(result.unresolved_decisions).toHaveLength(0);
    ...
    it("specs/.template/integration-ready.md의 placeholder는 가짜 통과시키지 않는다", async () => {
        ...
        writeFileSync(join(dir, "specs/.template/integration-ready.md"), "## 판정: {✅ AUTONOMOUS ZONE 진입 가능 / ❌ 미완료 항목 있음}\n", "utf-8");
        writeFileSync(join(dir, "tasks.md"), `## Phase 0.5\n### T0-001: 연동\n**status**: ☑\n**read**: -\n**write**: integration-ready.md\n**done**: file: integration-ready.md\n**deps**: -\n\n## Phase 1\n### T1-001: 구현\n**status**: ☐\n**read**: -\n**write**: a.ts\n**done**: file: a.ts\n**deps**: T0-001\n`, "utf-8");
        writeFileSync(join(dir, ".claude/state.json"), JSON.stringify({ ...DEFAULT_STATE, schema_version: STATE_SCHEMA_VERSION }), "utf-8");
        const result = await checkPhaseGate({ phase: 1 });
        expect(result.ok).toBe(false);
    ...
    it("Phase 1 진입 시에는 PROGRESS.md placeholder 검사 생략", async () => {
        ...
        mkdirSync(join(dir, ".claude"), { recursive: true });
        writeFileSync(join(dir, ".claude/state.json"), JSON.stringify({ ...DEFAULT_STATE, schema_version: STATE_SCHEMA_VERSION }), "utf-8");
        writeFileSync(join(dir, "PROGRESS.md"), "## 코드 패턴 메모\n\n{실제 프로젝트 구조 ...}\n{예: ...}", "utf-8");
        const result = await checkPhaseGate({ phase: 1 });
    ...
    function setupInsignProject(opts = {}) {
        ...
        writeFileSync(join(dir, "frontend/index.html"), `<script src="ngb_head.js"></script><script src="gnb.min.js"></script><script src="ngb_bodyend.js"></script>`, "utf-8");
        ...
        if (opts.mockAuthContext) {
            writeFileSync(join(dir, "frontend/src/context/AuthContext.tsx"), `const MOCK_USER = { name: "test" };\nlocalStorage.getItem('todo-mock-auth');`, "utf-8");
        }
        if (opts.storageState && opts.storageState !== "missing") {
            mkdirSync(join(dir, "tests/e2e/.auth"), { recursive: true });
            const expires = opts.storageState === "valid"
                ? Math.floor(Date.now() / 1000) + 86400
                : Math.floor(Date.now() / 1000) - 86400;
            writeFileSync(join(dir, "tests/e2e/.auth/user.json"), JSON.stringify({ cookies: [{ name: "_ifwt", expires }] }), "utf-8");
        }
        if (opts.activityLog) {
            mkdirSync(join(dir, "logs"), { recursive: true });
            writeFileSync(join(dir, "logs/boilerplate-activity.md"), "[SKILL] create-spec Step 2.7 user-approval — 2026-05-12\n", "utf-8");
        }
    ...
    it("auth_profile=insign + prerequisites.md OAuth 키워드 차단", async () => {
        ...
        writeFileSync(join(dir, "specs/001-app/prerequisites.md"), "| NCSR Callback URL | 🔴 차단 | ⬜ | redirect_uri 필요 |\n", "utf-8");
        writeFileSync(join(dir, ".claude/state.json"), JSON.stringify({
            ...DEFAULT_STATE,
            schema_version: STATE_SCHEMA_VERSION,
            auth_profile: "insign",
    ...
    it("auth_profile=nxas 면 OAuth 키워드 허용 (INSIGN 블랙리스트 적용 안 됨)", async () => {
        ...
        mkdirSync(join(dir, ".claude"), { recursive: true });
        mkdirSync(join(dir, "specs/001-app"), { recursive: true });
        writeFileSync(join(dir, "specs/001-app/prerequisites.md"), "| redirect_uri | 🔴 차단 | ⬜ | callback url |\n", "utf-8");
        writeFileSync(join(dir, ".claude/state.json"), JSON.stringify({
            ...DEFAULT_STATE,
            schema_version: STATE_SCHEMA_VERSION,
            auth_profile: "nxas",
...
//# sourceMappingURL=phase-gate.test.js.map