...
export function findTasksFile(cwd: string): string | null {
  const candidates = [
    "tasks.md",
    "specs/tasks.md",
  ];
...
export function parseTasks(tasksFilePath: string): ParsedTask[] {
  const content = fs.readFileSync(tasksFilePath, "utf-8");
  const lines = content.split("\n");

  const tasks: ParsedTask[] = [];
  let currentPhase = 0;
  let currentTask: Partial<ParsedTask> & { id: string } | null = null;
  // done 멀티라인 수집 상태
...
export function findTask(tasks: ParsedTask[], taskId: string): ParsedTask | undefined {
 ...
 */
export function warnIfMixedIdFormats(tasks: ParsedTask[]): void {
  ...
  if (withDash.length > 0 && withoutDash.length > 0) {
    process.stderr.write(
      `[d2a-harness] 경고: tasks.md에 Task ID 형식이 혼용되어 있습니다.\n` +
