...
import { findTasksFile, parseTasks, ParsedTask, normalizeTaskId } from "../shared/tasks-parser.js";
...
export interface NextTaskResult {
  ...
  skill: string | null;    // tasks.md **skill** 필드 — null이면 일반 구현 태스크
  all_done: boolean;       // Phase 내 모든 태스크 완료
  review_token_missing?: boolean;  // all_done=true 이면서 리뷰 토큰 미존재 경고
  error?: string;
}
...
export interface SubmitTaskResult {
  ...
  reason: string;
  next_task_id: string | null;   // action=next 일 때 다음 태스크 ID (없으면 Phase 완료)
 ...
 */
async function completedTaskIds(tasks: ParsedTask[]): Promise<Set<string>> {
  const state = await getState();
  const fromState = new Set((state.completed_tasks ?? []).map(normalizeTaskId));
  // tasks.md의 ☑ 상태도 포함 (두 소스 합산)
  for (const t of tasks) {
    if (t.status === "☑") fromState.add(normalizeTaskId(t.id));
  }
  return fromState;
}
...
function pickNextTask(
  ...
  for (const task of phaseTasks) {
    if (completed.has(normalizeTaskId(task.id))) continue;
    // deps도 정규화하여 비교
 ...
 */
async function autoResolveBlockers(cwd: string): Promise<void> {
  const state = await getState();
  if (!state.blockers || state.blockers.length === 0) return;

  const activeBlockers = state.blockers.filter((b) => {
    const match = b.reason.match(/recovery 브랜치: (recovery\/\S+)/);
    if (!match) return true; // recovery 브랜치 정보 없으면 유지
  ...
  if (activeBlockers.length < state.blockers.length) {
    ...
    process.stderr.write(
      `[d2a-harness] 블로커 자동 해제: ${resolved}개 (recovery 브랜치 소멸 감지)\n`
    );
...
function writeValidateToken(cwd: string): void {
 ...
 */
export async function getNextTask(args: { phase: number }): Promise<NextTaskResult> {
  const { phase } = args;
  ...
  try {
    ...
    return {
      ok: false, task_id: null, title: "", phase,
      read_files: [], write_files: [], deps: [], done_criteria: [],
      skill: null, all_done: false, error: `tasks.md 파싱 실패: ${(e as Error).message}`,
    };
  }
  ...
  if (allDone) {
    ...
    if (reviewTokenMissing) {
      process.stderr.write(
        `[d2a-harness] ⚠️  Phase ${phase} 완료: review token 없음 — ` +
...
function detectCriticalPathFiles(changedFiles: string[]): string[] {
  return changedFiles.filter((f) =>
    CRITICAL_PATH_PATTERNS.some((re) => re.test(f))
  );
}
...
export async function submitTask(args: {
  ...
  if (criticalFiles.length > 0) {
    process.stderr.write(
      `[d2a-harness] ⚠️  Critical Path 파일 감지 (${task_id}):\n` +
  ...
  if (validateResult.passed) {
    ...
    if (!completedTasks.includes(normalizedId)) completedTasks.push(normalizedId);
